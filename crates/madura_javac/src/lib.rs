    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoPlatformImage { exe } => write!(
                ...
                exe.as_deref().map(|p| p.to_string_lossy().into_owned()).as_deref().unwrap_or("<unknown>"),
            ),
...
fn platform_home(exe: &Path) -> Option<PathBuf> {
    ...
    root.join("lib").join("modules").is_file().then(|| root.to_path_buf())
}
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    let rc = unsafe { graal_create_isolate(std::ptr::null_mut(), &mut isolate, &mut thread) };
    if rc != 0 {
        return Err(InvokeError::IsolateInit(rc));
    }

    // No isolate teardown: the process exits right after this returns, and a
    // teardown would only add GC and unmap work to every invocation.
    Ok(unsafe {
        compile_javac(
            thread,
            bin.as_ref().map_or(std::ptr::null_mut(), |c| c.as_ptr().cast_mut()),
